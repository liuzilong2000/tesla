    	...
    	<div class="container" :class="{form:form}">	
			...
			<div class="content">
				...
				<form class="row no-gutters"  ref="input">
					...
					<div class="col-12">
						<div>
							<div class="label select"><label for="">省份</label></div>
							<select v-model="proStatus" :class="proFail" @change="proChange()">
								<option value="0">省份</option>
								<option v-for="(item,i) of proname" :key="i" :value="i+1">{{item.proname}}</option>
							</select>
					...
					<div class="col-12">
						<div>
							<div class="label select"><label for="">城市</label></div>
								<select v-model="cityStatus" :class="cityFail" @change="cityChange()">
								<option value="0">市(区)</option>
								<option v-for="(item,i) of cityname" :key="i" :value="i+1">{{item.cityname}}</option>
							</select>
							<span class="iconfont">&#xe6cc;</span>
							<p>{{cityMessage}}</p>
						</div>
...
/* 验证失败时边框红色 */
  ...
  #drive form+div button{
	...
	#drive .container:last-child{
		...
		#drive .container:last-child>div:first-child{
			width:80%;
			margin:0 auto
	}
	#drive .leftInp{
	...
	#drive .container:last-child>div>div{
		width:400px;
	}
	/* 左侧表单元素 */
